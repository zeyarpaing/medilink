...
export default function Page({ params }: { params: { type: 'hospitals' | 'clinics' } }) {
  ...
  return (
    <section className="mcontainer">
      ...
      <ul className=" grid grid-cols-1 gap-6 pb-12 md:grid-cols-3">
        {[...providers[providerType], ...providers[providerType].reverse(), ...providers[providerType]].map((plan) => (
          <Card className="w-full !shadow-sm" isFooterBlurred key={plan.name} shadow="none">
            ...
            <CardFooter className="justify-between border border-zinc-200 bg-white/60">
              <div className="w-full px-2 pb-2">
                <p className="mb-4 text-sm text-black">{plan.description}</p>
                <div className="text-right">
                  <Button className="px-8 text-sm" href={plan.name} color="primary" radius="full" size="md" as="a">
                    Visit
                  </Button>
                </div>
              </div>
